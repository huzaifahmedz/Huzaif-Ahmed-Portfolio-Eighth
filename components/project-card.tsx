...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <Link
      ...
      className="relative horizontal-line after:top-0 group"
    >
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="p-6 relative horizontal-line bg-background/40 backdrop-blur-xs grow flex flex-col">
          <h3
            ...
            className="font-cal text-2xl text-black dark:text-white group-hover:text-rose-500 transition-all line-clamp-2 min-h-[3.5rem]"
          >
            {project.title}
          </h3>
          <p
            ...
            className="text-muted-foreground text-base text-balance line-clamp-3 min-h-[4.5rem] mt-2"
          >
          ...
          <div className="flex items-center flex-wrap mt-5 gap-2">
            {Array.from({ length: 3 }).map((_, index) => (
              <Badge key={index} variant={'default'} className="animate-pulse">
                <Skeleton className="w-10 h-3 rounded-[1rem]" />
              </Badge>
            ))}
          </div>
        </div>
